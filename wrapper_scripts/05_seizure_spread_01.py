...
index = np.arange(start, start+duration,1/fs*1e6)
seizure_status = np.zeros(data.shape, dtype=int)
seizure_status = pd.DataFrame(seizure_status, columns=data.columns)

annotations = ds.get_annotations('seizure_state_andy')
ch = annotations[0].annotated[0].channel_label
ch_st = annotations[0].start_time_offset_usec
ch_end = annotations[0].end_time_offset_usec

ch_st_bool = index>ch_st
ch_end_bool = index<ch_end

ch_between_bool = ch_st_bool == ch_end_bool
index_true = np.where(ch_between_bool)[0]
seizure_status.iloc[  [np.where(ch_between_bool)],[1,2]  ]
seizure_status.loc[index_true,  ch  ] = 1
...
n_out=1024
NN='CNN'
features_description = 'Seizure_spread'
activation_s = 'tanh'
scaling_s = 'MinMax_HUP196_test01'
verbose = 1
training_epochs =20
batch_size = 2**1
learn_rate = 0.001
...
for iii in np.arange(0, 10, 1):
	print("This is iteration {0}".format(iii))
	start = 297945000000 #start of seizure
	duration = 100*1e6
	start = start + duration*iii
	print("Start Time: {0}".format(start/1e6))
	data, fs = iEEG_data_get('HUP196_phaseII', start, duration)
	...
	for qq in np.arange(0, len(start_elec)-1, 1):
		...
		if iii<1000:
			...
			series_plot_actual = pd.DataFrame({'Actual': train_y_all[0,np.arange(0, fs_downSample*2,1).astype(int)  ]})
